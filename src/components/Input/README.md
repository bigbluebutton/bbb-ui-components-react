...
- The action button reuses the shared `Button` component (`size="sm"`) for both the icon-only and labeled cases, so it fits a single-line row instead of imposing the fixed dimensions of `Button`'s `circle`/`stacked` layouts.
